	...
	deviationIntervals.forEach( (interval, i) => {
		avgVolumeForIntervals.slice(0, i+1).forEach( (avgi, j) => {
			...
			if (deviationHigh >= config.deviationFilterBars.price && deviationVolume >= config.deviationFilterBars.volume)
				console.log(`${symbol.symbol} | ${deviationIntervals[j]} - ${interval}: vol: ${deviationVolume.toFixed(2)}% | price: ${deviationHigh.toFixed(2)}%`, moment().format('MMMM Do YYYY, h:mm:ss a'), (deviationIntervals[j] === 10 && interval === 5) ? '*' : '')
		})
	})
}
